#!/usr/bin/env python3
"""
annotaion2.txt를 JSON으로 변환하여 IMG_2778.json에 저장하고 OCR 실행
"""
...
# Paths
annotation_txt = "data/everytime_samples/annotations/annotaion2.txt"
output_json = "data/everytime_samples/annotations/IMG_2778.json"
image_file = "IMG_2778.PNG"
image_path = f"data/everytime_samples/images/{image_file}"

# Step 1: annotaion2.txt 읽기 및 JSON 변환
...
print(f"\n📊 Ground Truth Schedule for IMG_2778:")
print(f"   - Total Entries: {len(annotation_json['schedule'])}")

# 요일별로 정렬해서 출력
days_order = ['월', '화', '수', '목', '금', '토', '일']
for day in days_order:
    day_entries = [e for e in annotation_json['schedule'] if e['day'] == day]
    if day_entries:
        print(f"\n   {day}요일:")
        for entry in day_entries:
            print(f"      {entry['start']}-{entry['end']}: {entry['class_name']}")

# Step 2: OCR 실행
print("\n" + "="*60)
print("🚀 Step 2: Running OCR on image IMG_2778.PNG...")
print("="*60)
